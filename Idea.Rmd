...
us.map$PADD[us.map$region %in% 
          c("alaska", "arizona","california", "colorado", "hawaii", 
            "idaho", "montana", "nevada", "new mexico", "oregon", "utah",
            "washington", "wyoming")] <- "PADD 2: West"
us.map$PADD[us.map$region %in% 
          c("connecticut", "maine", "massachusetts", "new hampshire", "new jersey", "new york", "pennsylvania", "rhode island","vermont")] <- "PADD 3: Northeast"
us.map$PADD[us.map$region %in% 
          c("illinois", "indiana", "iowa", "kansas", "michigan","minnesota","missouri", "nebraska", "north dakota", "ohio", "south dakota", "wisconsin")] <- "PADD 4: North Central"


# subset the dataframe by padd zones and move lat/lon accordingly
us.map$lat.transp[us.map$PADD == "PADD 1: South"] <- us.map$lat[us.map$PADD == "PADD 1: South"]
us.map$long.transp[us.map$PADD == "PADD 1: South"] <- us.map$long[us.map$PADD == "PADD 1: SouthSouth"] + 5

us.map$lat.transp[us.map$PADD == "PADD 2: West"] <- us.map$lat[us.map$PADD == "PADD 2: West"]
us.map$long.transp[us.map$PADD == "PADD 2: West"] <- us.map$long[us.map$PADD == "PADD 2: West"]
